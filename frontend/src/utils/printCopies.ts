...
export type CopyDefinition = { title: string; purpose: string };
...
export function printControlledCopies(documentId: string, secondParty: CopyDefinition) {
...
export async function printControlledCopySet(documentId: string, copies: CopyDefinition[]) {
  const source = document.getElementById(documentId);
  if (!source) throw new Error('Printable document is not available');
  if (source.dataset.outputAuthorized === 'false') throw new Error('This document cannot be printed until the external Finance handoff is authorized');
  if (!copies.length) throw new Error('At least one print copy is required');
  ...
  copies.forEach((copy, index) => {
    ...
    const marker = document.createElement('div'); marker.className = 'copy-control-marker';
    ...
    if (financeImage) {
      ...
      image.className = 'finance-print-image'; image.src = financeImage;
      image.alt = 'External Finance handoff package'; page.append(image, marker);
    } else {
      const clone = source.cloneNode(true) as HTMLElement; clone.removeAttribute('id');
      clone.querySelectorAll('[id]').forEach((node) => node.removeAttribute('id'));
      clone.classList.add('controlled-document-clone'); page.append(clone, marker);
    }
    root.appendChild(page);
  });
  document.body.appendChild(root); document.body.classList.add('dual-copy-print');
